...
const RV = RyView = {
    ...
    dtHelp: '我们平时常用的数据可视化视频的数据大多采用exl格式或csv格式又或者json格式，然而这些格式对用户并不友好，exl只能用相应的软件编辑，csv与json随便错一个字符就会导致数据错乱！所以在锐视里创作数据可视化视频的数据采用普通的文本格式，无论在哪里都可以编辑，文字排版也非常简单，通过简单的名值对录入数据即可。锐视里的可视化数据可用三维数据也可以用二维数据，而其它平台的只能用三维数据。如何理解二维与三维数据？可视化数据一般包含名称、数值与时间三个维度，例如一组展示多个城市每年GDP收入的数据，数据排列如下：北京:10w,上海:8w,重庆:6w,时间:2018; 北京:12w,上海:10w,重庆:8w,时间:2019; 北京:11w,上海:9w,重庆:10w,时间:2020; 像这种可视化数据每个时间段的数据为一帧，必须有很多帧才能形成动画，如果只有一帧的话看起来就是静止的画面。而锐视采用的可视化数据虽然也有三个维度，但是当只有一帧的时候时间维度是可以省略的，锐视的可视化数据并不是逐帧显示，每一帧都可以指定播放时间长度。<br><br>锐视的可视化数据由下列名值对组成：<br><br>t1 正标题<br><br>t2 副标题（时间维度）<br><br>tp 数据类型（1=可视化数据，2=台词数据，用于创作文字说视频）<br><br>nm 名称<br><br>vl 数值<br><br>可视化数据目前只用到名称与数值这两个属性，其它属性尚未开发，而在锐视的新版本中可能会通过其它方式实现。所以用户在组建数据时只需要指定名称与数值即可，其它属性可留空。每行数据由属性名加空格加属性值组成，而每当出现第二条时间属性时代表新一帧数据开始，时间属性以下的属性列表代表该时间维度的比拼数据。<br><br>用于创作文字说视频的台词数据目前尚未进行开发，新版本可能会以其它方式实现。',
    ...
    aad: (A) => { // add animation data
        ...
        b = RV.lg[l].ad;
        if(!a) {
            c = '<div class="rvBtn" onclick="RV.sn(RV.EB)">'+(RV.lg[l].inp||'输入(Input)')+'</div><div class="rvBtn">'+(RV.lg[l].loc||'本地(Local)')+'<input type="file" class="rvFile" onclick="RV.up(this)" multiple="1"/></div><div class="rvBtn" onclick="RV.ol(\'http://rv.eefaa.cn/?dnet\')">'+(RV.lg[l].net||'网络(Net)')+'</div><div class="rvBtn" onclick="RV.msg(\'数据模板\',RV.dtHelp)">'+(RV.lg[l].hp||'帮助(Help)')+'</div><b class="cl"></b>';
            RV.gn('rvTB2').innerHTML = c;
            return CL('no data');
        }
        ...
        f = '<div class="rvBtn" onclick="RV.sn(RV.EB)">'+(RV.lg[l].inp||'输入(Input)')+'</div><div class="rvBtn">'+(RV.lg[l].loc||'本地(Local)')+'<input type="file" class="rvFile" onclick="RV.up(this)" multiple="1"/></div><div class="rvBtn" onclick="RV.ol(\'http://rv.eefaa.cn/?dnet\')">'+(RV.lg[l].net||'网络(Net)')+'</div><div class="rvBtn" onclick="RV.msg(\'数据模板\',RV.dtHelp)">'+(RV.lg[l].hp||'帮助(Help)')+'</div>';
